...
def test(args, test_loader, model, criterion):
    ...
    for i, (input, label, size, name) in enumerate(test_loader):
        ...
        # run the mdoel
        print("model input size: ", input_var.size())
...
def test_model(args):
    ...
    if args.cuda:
        model = model.cuda()  #1-gpu-card for test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type = str, default = './checkpoint/camvid/CGNet_M3N21bs8gpu1_ontrainval/model_61.pth', 
    ...
    parser.add_argument("--gpus", type = str, default = "5",  help = "gpu ids (default: 0)")
